...
if [ "$(mongo ${db_name} --eval 'printjson(db.hosts.count({"name": "docker-host"}));' | tail -1 | tr -d '\r')" == "0" ]
then
  mongo ${db_name} --eval 'db.hosts.insert({"status" : "enabled", "work_base_dir" : "~/oacis_work", "mounted_work_base_dir" : "", "max_num_jobs" : 1, "polling_interval" : 5, "min_mpi_procs" : 1, "max_mpi_procs" : 1, "min_omp_threads" : 1, "max_omp_threads" : 1, "name" : "docker-host"})'
fi
